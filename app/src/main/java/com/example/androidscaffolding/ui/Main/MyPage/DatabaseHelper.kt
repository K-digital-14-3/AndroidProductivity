...
class DatabaseHelper(context: Context?) : SQLiteOpenHelper
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("create table " + TABLE_NAME +
                "(ID INTEGER PRIMARY KEY AUTOINCREMENT, EMAIL TEXT, PASSWORD TEXT, PROFILEURI TEXT)")
    }
    ...
    // 데이터베이스 추가하기 insert
    fun insertData(email: String?, password: String?, profileUri: String?): Boolean {
        ...
        contentValues.put(COL_4, profileUri)
    ...
    //데이터베이스 수정하기
    fun updateData(id: String, email: String?, password: String?, profileUri: String?): Boolean {
        val db = this.writableDatabase
        val contentValues = ContentValues()
    ...
    companion object {
        ...
        const val DATABASE_NAME = "TESTWORLD.db" // 데이터베이스 명
